...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		 ...
		 * .POST, "/login").permitAll() .anyRequest().authenticated().and() // Filter
		 * for the api/login requests .addFilterBefore(new LoginFilter("/login",
		 * authenticationManager()), UsernamePasswordAuthenticationFilter.class) //
		 * Filter for other requests to check JWT in header .addFilterBefore(new
